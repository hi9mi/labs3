...
public class Main {
    public static void main(String[] args) {
        ...
        int K = readPositiveInt(sc, "Введите K (кол-во вагонов): ");
        int M = readPositiveInt(sc, "Введите M (кол-во мест в вагоне): ");
        int[][] sold = new int[K][M];

        int N = readNonNegativeInt(sc, "Введите N (кол-во проданных билетов): ");
        readSoldPairs(sc, sold, N);
    ...
    private static void readSoldPairs(Scanner sc, int[][] sold, int N) {
        int K = sold.length, M = sold[0].length;
        System.out.println("Введите пары <вагон место> (нумерация с 1):");
        for (int i = 0; i < N; i++) {
            int car = sc.nextInt(), seat = sc.nextInt();
            if (car < 1 || car > K || seat < 1 || seat > M) {
                System.out.printf("Диапазоны: вагон 1..%d, место 1..%d. Повторите\n", K, M);
                i--;
                continue;
            }
            sold[car - 1][seat - 1] = 1;
    ...
    private static void printSummary(int[][] sold) {
        ...
        for (int car = 0; car < K; car++) {
            ...
            totalFree += free;
            System.out.printf("Вагон %d: свободных %d из %d — %s%n",
                    car + 1, free, M, free > 0 ? "ЕСТЬ" : "НЕТ");
        }
    ...
    private static void queryLoop(Scanner scanner, int[][] sold) {
        ...
        while (true) {
            ...
            System.out.printf("Вагон %d: свободных мест %d — %s%n",
                    q, free, free > 0 ? "ЕСТЬ" : "НЕТ");
        }
    }
}
