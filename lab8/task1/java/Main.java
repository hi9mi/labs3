...
public class Main {
  ...
  enum CarType {
    ...
    static CarType fromCode(int code) {
      for (CarType c : values()) if (c.code == code) return c;
      return OTHER;
    }
  ...
  public static void main(String[] args) {
    ...
    for (Product product : productList) {
      ...
      System.out.println(
        "Цена в долларах: " +
          String.format(Locale.US, "%.2f", product.toUSD(100))
      );
    ...
    for (Product product : productList) product.output();
  }
}
...
class Product {
  ...
  public void input(Scanner scanner) {
    ...
    if (manufacturer.isEmpty()) manufacturer = "Неизвестный изготовитель";
    ...
    System.out.print(
      "Введите дату создания (дд.мм.гггг) или оставьте пустым для текущей: "
    );
    ...
    if (dateStr.isEmpty()) {
      ...
      if (parsed == null) {
        System.out.println(
          "Некорректный формат даты — установлена текущая дата."
        );
        creationDate = Calendar.getInstance();
      } else {
        creationDate = parsed;
  ...
  public void output() {
    System.out.println(
      "Товар: " +
        ...
        carType +
        ", Дата создания: " +
        SDF.format(creationDate.getTime())
    );
  }
  ...
  public void increaseIfToyota(double rate, double increase) {
    if (
      name != null && name.toLowerCase(java.util.Locale.ROOT).contains("toyota")
